...
class AntibodyComplexDataset(FairseqDataset):
    ...
    def __init__(self, data_path, split, seq_vocab, tag_vocab, cdr_types, L_target, pad_idx, mask_idx, max_len,savepdb=False):
    ...
    def __getitem__(self, idx):
        ...
        if self.savepdb:
            return item['masked_antibody_seq'], item['antibody_cdr'], item['label_antibody_seq'], \
                ...
                item['antibody_coords'], item['antibody_seq'], item['cdr_string'],item['pdb']
        else:
            return item['masked_antibody_seq'], item['antibody_cdr'], item['label_antibody_seq'], \
                   item['paratope_seq'], item['paratope_coords'], item['paratope_atypes'], \
                   item['antigen_seq'], item['antigen_coords'], item['antigen_atypes'], \
    ...
    def collate_fn(self, batch):
        try:
            if self.savepdb:
                masked_antibody_seq, antibody_cdr, label_antibody_seq, \
                    ...
                    antibody_coords, antibody_seq, cdr_string, pdb \
                    = tuple(zip(*batch))
            else:
                masked_antibody_seq, antibody_cdr, label_antibody_seq, \
                paratope_seq, paratope_coords, paratope_atypes, \
        ...
        if self.savepdb:
            return batched_seq, batched_tag, batched_label, paratope, epitope, antibody, antigen_seq, antigen_coords, antibody_coords, antibody_seq, cdr_string, pdb
        else:
