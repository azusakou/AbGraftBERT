...
class AntibodyRobertaModel(FairseqEncoderModel):
    def __init__(self, args, encoder, dictionary, inference=False):
        ...
        if self.args.finetune:
            ...
            unfreeze_module_params(self.encoder.lm_head)
...
class RobertaEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, tag_dict):
        ...
        self.read_lm_from_ptfile()

    def read_lm_from_ptfile(self):
        ...
        layers_state_dict = {key.replace('encoder.lm_head.', ''): value for key, value in state_dict['model'].items() if
                          'encoder.lm_head' in key}
        self.lm_head.load_state_dict(layers_state_dict, strict=True)
        del state_dict
    ...
    def forward(
        ...
        **unused
        ...
        #print(attention.transpose(0, 1).mean(dim=0)[masked_tokens.transpose(0, 1)])
        #print(attention.transpose(0, 1)[0][masked_tokens.transpose(0, 1)])
    ...
    def extract_features(self, src_tokens, tag_tokens, return_all_hiddens=False, past_key_values=None, **kwargs):
        inner_states, sentence_rep, att = self.sentence_encoder(
            src_tokens,
        ...
        return features, {"inner_states": inner_states if return_all_hiddens else None}, sentence_rep, att
