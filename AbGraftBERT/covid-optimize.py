...
class CovidNeutralizationModel():
    ...
    def __init__(self):
        MODEL_PATH = "checkpoints/exp3/neut_model.ckpt"
        MODEL_ARGS = {
            "hidden_dim": 256,
            "n_layers": 2,
            "use_srupp": False
...
def load_data():
    # Load antibody data
    with open("data/finetune/exp3-covabdab/CoV-AbDab_050821.csv", "r") as f:
...
def make_entry_pre(d, args, igfold):
...
def make_entry(d, args, igfold):
    ...
    parser = PDBParser()
    structure = parser.get_structure(entry['name'], pred_pdb)
    model = structure[0]  # Assuming only one model in the PDB file

    # Extract coordinates
    chain_id = 'H'  # Assuming the VH chain is denoted by 'H' in the PDB file
    coords = []
...
def load_checkpoint(args, model,nepoch):
    checkpoint_ckpt = os.path.join(args.save_dir, f"model.ckpt.{nepoch}")
    model.load_state_dict(torch.load(checkpoint_ckpt)[0])
    return model
...
def main(args):
    ...
    lr_scheduler = lambda epoch: args.lr * epoch / 500 if epoch < 500 else (
        args.lr if epoch <= 10000 else args.lr * (1 + math.cos(math.pi * epoch / args.epochs)) / 2)
    ...
    evaluator[0].eval()
    model_ckpt = torch.load("checkpoints/exp3/hieratt.ckpt")[0]
    ...
    evaluator[1].eval()
    model_ckpt = torch.load("checkpoints/exp3/lstm.ckpt")[0]
    ...
    evaluator[2].eval()
    model_ckpt = torch.load("checkpoints/exp3/autoreg.ckpt")[0]
    ...
    for e in pbar:
        # Decode new cdrs
        ppl_list = []
        ab = random.choice(train_ab); append_count = 0
        name = ab['name']
        ...
        for new_cdr, ppl in zip(new_seqs, prior_ppl):
            if is_valid_seq(new_cdr) and ppl <= args.max_prior_ppl:
                ...
                if prob > entry['score']: # first round: about 24 in 200
                    ...
                    entry['hcdr3'] = new_cdr
                    entry['seq'] = entry['VH'] if args.architecture == 'hierarchical' else entry['hcdr3']
                    #if (append_count == 0) and (len(train_data[name]) == 1):
                    #    train_data[name][-1]['hcdr3'] = entry['hcdr3']
                    #    train_data[name][-1]['seq'] = entry['seq']
                    #    train_data[name][-1]['score'] = entry['score']
                    #else:
        ...
        pbar.set_description(f'lr: {round(pg["lr"],5)}; loss: {round(loss.item(),5)}; PPL mean: {round(np.array(prior_ppl).mean(),1)};  PPL min: {round(np.array(prior_ppl).min(),1)}; PPL ava: {round(np.array(ppl_list).mean(),1)}')
        if ((e + 1) % args.valid_iter == 0):
            ...
            batch_size_backup = int(args.batch_size)
            args.batch_size = 100
            ...
            print(args.batch_size)
        #optimizer.set_lr(lr_scheduler(e))
    # best_epoch = 10000
    if best_epoch > 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cktpath", type=str, default="checkpoints/exp3/pflen5_iter5_loss1_1_2_lr0.0005_bsz24_seed128/checkpoint_last.pt")
    ...
    parser.add_argument("--sars_cov", type=bool, default=True)
